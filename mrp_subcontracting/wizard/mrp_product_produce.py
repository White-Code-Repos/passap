...
class MrpProductProduce(models.TransientModel):
        ...
        if self.consumption == 'strict':
            for move in self.production_id.move_raw_ids:
                ...
                if float_compare(qty_done, qty_to_consume, precision_rounding=rounding) != 0:
                    raise UserError(_(
                        'You should consume the quantity of %s defined in the BoM. If you want to consume more or less components, change the consumption setting on the BoM.') %
    ...
    def _generate_lines_values(self, move, qty_to_consume):
        ...
        for move_line in move.move_line_ids:
            ...
            if move_line.lot_produced_ids or float_compare(move_line.product_uom_qty, move_line.qty_done,
            ...
            if linked_wo_line:
                if float_compare(sum(linked_wo_line.mapped('qty_to_consume')),
                                 move_line.product_uom_qty - move_line.qty_done,
                                 precision_rounding=move.product_uom.rounding) < 0:
                    to_consume_in_line = min(qty_to_consume, move_line.product_uom_qty - move_line.qty_done - sum(
            ...
            if not line.move_id:
                ...
                if not move_id:
                    ...
                    if line.raw_product_produce_id:
                        ...
                        values = self.production_id._get_finished_move_value(line.product_id.id, 0,
...
class MrpProductProduceLine(models.TransientModel):
    ...
    def _get_produced_lots(self):
        return self.move_id in self._get_production().move_raw_ids and self._get_final_lots() and [(4, lot.id) for lot
                                                                                                   in
                                                                                                   self._get_final_lots()]
