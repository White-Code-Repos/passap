...
class StockMove(models.Model):
    ...
    def action_show_details(self):
        ...
        if self.is_subcontract:
            ...
            if self._has_tracked_subcontract_components() and \
                    float_compare(production.qty_produced, production.product_uom_qty,
        ...
        for move in self:
            ...
            move.write({
                ...
                'location_id': move.picking_id.partner_id.with_context(
                    force_company=move.company_id.id).property_stock_subcontractor.id
            })
    ...
    def _check_overprocessed_subcontract_qty(self):
        ...
        for move in self:
            ...
            if float_compare(move.quantity_done, move.move_orig_ids.production_id.qty_produced,
...
operations.""") % ('\n'.join(overprocessed_moves.mapped('product_id.display_name'))))
    ...
    def _has_tracked_subcontract_components(self):
        for item in self:
            item.ensure_one()
            for move in item.move_orig_ids:
                return any(m.has_tracking != 'none' for m in move.production_id.move_raw_ids)
