...
class TestSubcontractingBasic(TransactionCase):
    def test_subcontracting_location_1(self):
        ...
        company2 = self.env['res.company'].create(
            {'name': 'Test Company'})  # Test fail to create a new company that way
...
class TestSubcontractingFlows(TestMrpSubcontractingCommon):
    def test_flow_1(self):
        ...
        self.assertEqual(return_picking.move_lines.location_dest_id,
                         self.subcontractor_partner1.property_stock_subcontractor)
    ...
    def test_flow_7(self):
        ...
        self.assertTrue(all(p[2]['product_id'] in (self.comp1 | self.comp2).ids for p in
                            register_form._values['raw_workorder_line_ids']),
        ...
        details_subcontract_moves_form = Form(move_raw_comp_1,
                                              view=self.env.ref('mrp_subcontracting.mrp_subcontracting_move_form_view'))
        ...
        details_subcontract_moves_form = Form(move_raw_comp_2,
                                              view=self.env.ref('mrp_subcontracting.mrp_subcontracting_move_form_view'))
        for i in range(len(details_subcontract_moves_form._values['move_line_ids'])):
            with details_subcontract_moves_form.move_line_ids.edit(i) as sc:
                if sc._values['qty_done']:
        ...
        self.assertEqual(move_comp1.move_line_ids.filtered(lambda ml: not ml.product_uom_qty).lot_produced_ids.name,
                         'LOT F2')
        self.assertEqual(move_comp2.move_line_ids.filtered(lambda ml: not ml.product_uom_qty).lot_produced_ids.name,
                         'LOT F2')
